...
export interface Message {
  /**
   * Optional context ID the message is associated with.
   * @default null
   */
  contextId?: string | null;

  /**
   * Optional flag indicating if this is the final message in a stream.
   * @default null
   */
  final?: boolean | null;

  /**
   * Unique identifier created by the message creator.
   */
  messageId: string;
  ...
  /**
   * Optional task ID the message is related to.
   * @default null
   */
  taskId?: string | null;
...
/**
 * Parameters for sending a message. Sent by the client to the agent as a request.
...
export interface MessageSendParams {
   ...
   */
  configuration?: MessageSendConfiguration | null;
...
export interface SendStreamingMessageRequest extends JSONRPCRequest {
   ...
   */
  /**
   * Method name for sending a message and subscribing to updates.
   */
  method: "message/stream";
  /**
   * Parameters for the streaming message send method.
   */
  params: MessageSendParams;
}

export type SendMessageResponse = JSONRPCResponse<Message | Task | null, A2AError>;

export type SendMessageStreamingResponse = JSONRPCResponse<Event | null, A2AError>;
